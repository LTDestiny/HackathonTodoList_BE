...
class TaskService {
    static async createTask(userId, data) {
        return await db_1.prisma.task.create({
            data: {
                ...data,
                userId,
            },
    ...
    static async getTasks(userId, query) {
        const { page, limit, search, category, priority, status, sortBy, sortOrder, } = query;
        const where = { userId };
        if (search) {
            where.OR = [
    ...
    static async getTaskById(userId, taskId) {
        return await db_1.prisma.task.findFirst({
    ...
    static async updateTask(userId, taskId, data) {
        ...
        return await db_1.prisma.task.update({
            where: { id: taskId, userId },
            data: updateData,
            include: {
                category: true,
    ...
    static async deleteTask(userId, taskId) {
        return await db_1.prisma.task.delete({
            where: { id: taskId, userId },
        });
    }
    ...
    static async getTasksByUrgency(userId) {
        const now = new Date();
    ...
    static suggestDeadline(priority, estimateMinutes) {
        const now = new Date();
